...
export function getFirstViewablePage(user) {
    if (isMasterAdmin(user?.role)) return '/Admin';
    const key = PAGE_KEYS.find((page) => page !== 'Welcome' && hasPermission(user, page, 'view'));
    return key ? `/Admin/${key}` : '/Admin/TableManage';
}
